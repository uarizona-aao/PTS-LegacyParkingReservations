...
class database
{
	...
	private function checkLogin() {
		...
		if (isset($_SESSION['custcreate']) && (isset($_POST['custcreate']) || isset($_POST['deptcreate']))) {
			...
			//############################### Login Class ##################################
			include_once '/var/www2/include/gr/login_gr.php';
	...
	{
		/***
		 * For garage resservations, for logging in via webauth (or ldap),
		 *		and also queries tables of the form PARKING.GR_*
		 * $type is access levels: 1, 2, 3, or 4
		 */
		global $dbConn;
		...
		if (checkEnabled('Webauth Garage'))
		{
			...
			{
				// not logged in (or maybe not in auth table)
				locationHref('https://parking.arizona.edu/parking/garage-reservation/index.php?res_from_db=1');
				// this shows up for a second during login, so make it friendly
				exitWithBottom('If you are having Login Errors, '.CONTACT_CR);
			}
		}
		else
		{
