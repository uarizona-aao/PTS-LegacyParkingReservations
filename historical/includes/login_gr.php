...
/***
 * The login class is used only once, in database .php
 */
class login
{
	var $db;
	...
	function __construct ($Pnetid, $pass, $db_conn)
	{
		...
		if ($Pnetid)
		{
			...
			else
			{
				...
				//if (!$ds) {
				//	$this->error = 'netidconn';
				//	return false;
				//}
				//// bind to the LDAP server
				//$dn = 'uid='.$Pnetid.',ou=Accounts,ou=NetID,ou=CCIT,o=University of Arizona,c=US';
				//$bind = @ldap_bind($ds, $dn, $pass);
			}

			// if bind failed, then this means it's a username or password error
			if (!$bind)
			{
				$this->error = 'ldapBind';
				return false;
			...
			{
				// only continue working with LDAP if the bind was successful meaning a successful login
				if (checkEnabled('Webauth Garage'))
				{
				...
				{
					//// now, look up the username and password
					//$sr = ldap_search($ds, $dn, '(objectclass=*)');
					//$results = ldap_get_entries($ds, $sr);
					//
					//// get the db key for potential phonebook look-up
