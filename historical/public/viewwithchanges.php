...
if ($auth >= 2) // $_SESSION['cuinfo']['auth'] must be >= 2
{
	...
	if (count($res->otherDates) > 1)
	{
		foreach ($res->otherDates as $k_resid => $v_date)
		{
			if ($res->conf == $k_resid)
				...
				echo ", &nbsp;&nbsp;<a href='view.php?id=$k_resid' class='thetip' title='Reservation ID ".$res->conf."'>$v_date</a>";
		}
	}
	echo "</div>\n";
	...
	if (preg_match('/bio.?med/si', $res->garageName))
	{
	...
	{
		$realGarageStr = $res->garageName;
	}

	echo "<p><b>Garage:</b> ".$realGarageStr."</p>\n";
	...
	echo "<p><b>Guests May Come and Go:</b> ".(($res->comego) ? 'Yes' : 'No');
