...
function get_daily() {
	...
	$query = "SELECT COUNT(EX_ID) AS TALLY FROM PARKING.GR_GUEST_EXCEPTION WHERE TRUNC(EX_TIME)>=(TRUNC(SYSDATE))-30 AND EX_GARAGE_FK=$garage";
	$db->query($query);
	$results = $db->get_results();
	$schedule .= '<p class="center"><b>Current Exception Tally:</b> '.$results[0]['TALLY']."</p>\n";

	$query = "select RESERVATION_ID, DEPT_NAME,DEPT_NO_FK,RES_DATE, (select sum(GROUP_SIZE) from PARKING.GR_GUEST where RESERVATION_ID_FK = RESERVATION_ID) SPACES, to_char(RES_DATE, 'MM/DD/YYYY') R_DATE, to_char(ENTER_TIME, 'HH:MI AM') ENTER_TIME, to_char(EXIT_TIME, 'HH:MI AM') EXIT_TIME, ALLOW_EXTRA, COME_AND_GO
	from PARKING.GR_RESERVATION, PARKING.GR_DEPARTMENT, PARKING.GR_USER
	where USER_ID_FK = USER_ID and DEPT_NO = DEPT_NO_FK and GARAGE_ID_FK = $garage and trunc(RES_DATE) >= (trunc(sysdate))-30 and ACTIVE = 1
	order by RES_DATE, to_date(ENTER_TIME, 'HH:MI AM')";
	$db->query($query);
	...
	foreach($results as $result) {
 		...
 		$schedule .= '<h2>'.makeHtmlSpecial($result['ENTER_TIME']).' - '.makeHtmlSpecial($result['EXIT_TIME']).' - '.makeHtmlSpecial($result['RES_DATE']).'</h2>';
		...
		$query = "select REPLACE(GUEST_NAME,' & ',' &amp; ') AS GUEST_NAME, GROUP_EXITED, GROUP_SIZE, ADDON from PARKING.GR_GUEST where RESERVATION_ID_FK = $id order by GROUP_EXITED, upper(SORT_NAME)";
		$db->query($query);
		...
		// It's a guest list
		      ...
		      $schedule .= '</ul>';
		      if($allow_extra) {
		      	 $query = "select sum(ADDON) ADDS from PARKING.GR_GUEST where RESERVATION_ID_FK = $id";
		          $db->query($query);
		...
		$query = "SELECT REPLACE(NOTE,' & ',' &amp; ') AS NOTE FROM PARKING.GR_RESERVATION_NOTE WHERE RESERVATION_ID_FK=$id AND CASHIER_DISPLAY=1";
		if($db->query($query)) {
...
?>
<p style="text-align:center;"><a href="/parking/garage-reservation/cashier/index2.php">&lt;&lt; Back to Main Cashier Screen</a></p>
