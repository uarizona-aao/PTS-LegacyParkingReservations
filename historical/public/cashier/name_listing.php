...
function get_namelist() {
    ...
    $query = "select GUEST_NAME, GROUP_SIZE, GROUP_EXITED, INITIALS, RESERVATION_ID, COME_AND_GO from PARKING.GR_GUEST, PARKING.GR_RESERVATION, PARKING.GR_GUEST_INITIALS where GR_GUEST.RESERVATION_ID_FK = RESERVATION_ID and GR_GUEST.RESERVATION_ID_FK = GR_GUEST_INITIALS.RESERVATION_ID_FK (+) and GUEST_NAME = GR_GUEST_INITIALS.GUEST_NAME_FK (+) and trunc(RES_DATE) = trunc(sysdate) and ACTIVE = 1 and GROUP_SIZE = 1 and GARAGE_ID_FK = $garage order by upper(SORT_NAME)";
    $schedule .= get_listing($query, $garage);

    // Group List
    $schedule .= "<h2>Groups</h2>";
    $query = "select GUEST_NAME, GROUP_SIZE, GROUP_EXITED, RESERVATION_ID from PARKING.GR_GUEST, PARKING.GR_RESERVATION where RESERVATION_ID_FK = RESERVATION_ID and trunc(RES_DATE) = trunc(sysdate) and ACTIVE = 1 and GROUP_SIZE > 1 and GARAGE_ID_FK = $garage order by upper(SORT_NAME)";
    $schedule .= get_listing($query, $garage);
