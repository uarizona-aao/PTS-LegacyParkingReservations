...
function get_daily() {
	...
	foreach($results as $result) {
	  $schedule .= '<h2>'.$result['ENTER_TIME'] .' - '. $result['EXIT_TIME'] . '</h2>';
	  $spaceword = $result['SPACES'] == 1 ? 'space' : 'spaces';
	  /*if ($garage!=3) */$schedule .= '<p>'.$result['DEPT_NAME'].' ('.$result['SPACES']." $spaceword)</p>";
	//else $schedule .= "<p>".$result['DEPT_NO_FK']."</p>";
	  $comeandgo = $result['COME_AND_GO'];
	  $allow_extra = $result['ALLOW_EXTRA'];
	  ...
	  // It's a guest list
	      ...
	      foreach($group as $person) {
	          ...
	          $exited = $person['GROUP_EXITED'];
	          $size = $person['GROUP_SIZE'];
