...
function get_daily() {
	...
	$query = "SELECT COUNT(EX_ID) AS TALLY FROM PARKING.GR_GUEST_EXCEPTION WHERE TRUNC(EX_TIME)>=(TRUNC(SYSDATE))-1 AND EX_GARAGE_FK=$garage";
	$db->query($query);
	$results = $db->get_results();
	$schedule .= '<p class="center"><b>Current Exception Tally:</b> '.$results[0]['TALLY']."</p>\n";

	$query = "select RESERVATION_ID, DEPT_NAME,DEPT_NO_FK, (select sum(GROUP_SIZE) from PARKING.GR_GUEST where RESERVATION_ID_FK = RESERVATION_ID) SPACES, to_char(RES_DATE, 'MM/DD/YYYY') R_DATE, to_char(ENTER_TIME, 'HH:MI AM') ENTER_TIME, to_char(EXIT_TIME, 'HH:MI AM') EXIT_TIME, ALLOW_EXTRA, COME_AND_GO
	from PARKING.GR_RESERVATION, PARKING.GR_DEPARTMENT, PARKING.GR_USER
	where USER_ID_FK = USER_ID and DEPT_NO = DEPT_NO_FK and GARAGE_ID_FK = $garage and trunc(RES_DATE) >= (trunc(sysdate))-1 and ACTIVE = 1
