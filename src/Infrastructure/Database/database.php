...
// Should load $_ENV if not already done.
...
class database
{
	...
	private function selectDb($conn_name)
	{
		...
		switch ($conn_name)
		{
			...
			default:
				/***
				 * Was previously on a more complicate env; this is now handled through env variables.
				 */
				$d_conns['dbHost'] = $_ENV['DB_HOST'];
				$d_conns['serviceName'] = $_ENV['DB_SERVICE_NAME'];
				$d_conns['uuu'] = $_ENV['DB_USERNAME']; // parking    flexadmin
				$d_conns['ppp'] = $_ENV['DB_PASSWORD']; // mis4pts    flexadmin4pts23
	...
	private function checkLogin()
	{
		...
		elseif (isset($_POST['login']) && !isset($_GET['logout']))
		{
			...
			$login = new GrLogin($_POST['login'], $_POST['password'], $this);
